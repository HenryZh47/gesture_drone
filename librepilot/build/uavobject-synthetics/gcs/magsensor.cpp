 ...
 */
void MagSensor::setData(const DataFields& data, bool emitUpdateEvents)
{
    QMutexLocker locker(mutex);
    // Get metadata
 ...
 */
void MagSensor::registerQMLTypes() {
    qmlRegisterType<MagSensor>("UAVTalk.MagSensor", 1, 0, "MagSensor");
    qmlRegisterType<MagSensorConstants>("UAVTalk.MagSensor", 1, 0, "MagSensorConstants");

}

float MagSensor::x() const
{
   QMutexLocker locker(mutex);
   return static_cast<float>(data_.x);
}
void MagSensor::setX(const float value)
{
   mutex->lock();
...
float MagSensor::y() const
{
   QMutexLocker locker(mutex);
   return static_cast<float>(data_.y);
}
void MagSensor::setY(const float value)
{
   mutex->lock();
...
float MagSensor::z() const
{
   QMutexLocker locker(mutex);
   return static_cast<float>(data_.z);
}
void MagSensor::setZ(const float value)
{
   mutex->lock();
...
float MagSensor::temperature() const
{
   QMutexLocker locker(mutex);
   return static_cast<float>(data_.temperature);
}
void MagSensor::setTemperature(const float value)
{
   mutex->lock();
